...
export class CustomerService {
  ...
  constructor(
    ...
    @Inject('CUSTOMER_REPOSITORY') private readonly customerRepository: Repository<CustomerProfile>,
  
  ){}


  //SAVE CUSTOMER
  async createCustomerProfiles(user: User, queryRunner: QueryRunner): Promise<CustomerProfile>{
    ...
    const customerProfile = await queryRunner.manager.save(CustomerProfile, {
      user: user
    });
  ...
  //EDIT PROFILE
  async editProfiles(userEntity: User, updateProfileDto: UpdateProfileDto): Promise<CustomerProfile>{
  ...
  async getCustomerProfilesByUserId(userEntity: User): Promise<CustomerProfile>{
  ...
  private async oneProfile(userEntity: User): Promise<CustomerProfile>{
